...
stop_process() {
  local name="$1" signal="${2:-TERM}" pid_file="$ROOT_DIR/.runtime/$1.pid"
  [[ -f "$pid_file" ]] || return 0
  local pid
  pid="$(cat "$pid_file")"
  if kill -0 "$pid" 2>/dev/null; then
    ...
    kill -0 "$pid" 2>/dev/null && { echo "$name is still stopping" >&2; exit 1; }
  fi
  rm -f "$pid_file"
}
start_process() {
...
# Every invocation is a new capture. Preserve Kafka/core volumes, but discard
# the previous Inspector projection and use a group with no historical offset.
stop_process api
if [[ "${1:-}" == --with-radio ]]; then
  stop_process ue INT
  stop_process gnb
fi
capture_dir="$ROOT_DIR/rca-ingester/out/live"
mkdir -p "$capture_dir"
find "$capture_dir" -mindepth 1 -maxdepth 1 -delete
capture_group="${KAFKA_GROUP_ID:-5g-inspector-$(date -u +%Y%m%dT%H%M%SZ)-$$}"
printf '%s\n' "$capture_group" > "$ROOT_DIR/.runtime/capture-group"
cd "$ROOT_DIR/rca-ingester"
start_process api .venv/bin/python -m s5.api --port "$API_PORT" --brokers "${KAFKA_BROKERS:-localhost:9094}" --group-id "$capture_group"
...
if [[ "${1:-}" == --with-radio ]]; then
  ...
  cd "$ROOT_DIR/openairinterfaceue/cmake_targets/ran_build/build"
